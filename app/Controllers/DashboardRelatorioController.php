...
class DashboardRelatorioController extends DashboardController
{
  ...
  public function visualizacoes()
  {
    ...
    if (isset($_GET['data_inicio']) and ! isset($_GET['data_fim'])) {
      ...
      if ($dataInicio) {
        $condicoes[] = ['campo' => 'Visualizacao.criado', 'operador' => 'BETWEEN', 'valor' => [(new DateTime($dataInicio))->format('Y-m-d H:i:s'), (new DateTime('now'))->format('Y-m-d H:i:s')]];
      }
    }
    ...
    if (isset($_GET['data_fim']) and ! isset($_GET['data_inicio'])) {
      ...
      if ($dataFim) {
        $condicoes[] = ['campo' => 'Visualizacao.criado', 'operador' => '<=', 'valor' => (new DateTime($dataFim))->format('Y-m-d H:i:s')];
      }
    }
    ...
    if (isset($_GET['data_inicio']) and isset($_GET['data_fim'])) {
      ...
      // Sempre depois do filtro
      if ($dataInicio and $dataFim) {
        $condicoes[] = ['campo' => 'Visualizacao.criado', 'operador' => 'BETWEEN', 'valor' => [(new DateTime($dataInicio))->format('Y-m-d H:i:s'), (new DateTime($dataFim))->format('Y-m-d H:i:s')]];
      }
    }
    ...
    $botaoVoltar = $this->obterReferer();
    $resultado = $this->dashboardRelatorioModel->buscarVisualizacoes($condicoes);
