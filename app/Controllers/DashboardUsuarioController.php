...
class DashboardUsuarioController extends DashboardController
{
  ...
  public function usuarioEditarVer(int $id)
  {
    ...
    if (isset($usuario['erro']) and $usuario['erro']) {
      $this->redirecionarErro('/dashboard/usuarios/' . $this->usuarioLogado['empresaId'], $usuario['erro']);
    }
  ...
  {
    if ($this->usuarioLogado['padrao'] != USUARIO_SUPORTE) {
      $this->redirecionarErro('/dashboard/usuarios/' . $this->usuarioLogado['empresaId'], 'Você não tem permissão para realizar esta ação.');
    }
    ...
    if (isset($resultado['erro'])) {
      $this->redirecionarErro('/dashboard/usuario/editar/' . $this->usuarioLogado['empresaId'] . $id, $resultado['erro']);
    }

    $this->sessaoUsuario->apagar('acessoBloqueado-' . $id);
    $this->redirecionarSucesso('/dashboard/usuario/editar/' . $this->usuarioLogado['empresaId'] . $id, 'Acesso desbloqueado com sucesso');
  }
  ...
  public function adicionar(): array
  {
    ...
    if (isset($resultado['erro'])) {
      $this->redirecionarErro('/dashboard/usuario/adicionar/' . $this->usuarioLogado['empresaId'], $resultado['erro']);
    }

    $this->redirecionarSucesso('/dashboard/usuarios/' . $this->usuarioLogado['empresaId'], 'Usuário criado com sucesso');
  }
  ...
  public function atualizar(int $id)
  {
    ...
    $this->redirecionarSucesso('/dashboard/usuario/editar/' . $this->usuarioLogado['empresaId'] . '/' . $id, 'Registro alterado com sucesso');
  }
