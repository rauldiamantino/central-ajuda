...
<div class="mt-2 py-2 h-max flex flex-col gap-2 md:gap-1 md:flex-row justify-start items-center">
  <div class="flex justify-center items-center gap-1">
    <?php if ($pagina > 1) { ?>
      <a href="<?php echo baseUrl('/dashboard/' . $this->usuarioLogado['empresaId'] . '/usuarios?pagina=' . $pagina - 1); ?>" class="border-2 border-slate-200 hover:border-slate-500 text-gray-500 p-3 rounded-lg">
    ...
    <?php } ?>
    <form action="<?php echo baseUrl('/dashboard/' . $this->usuarioLogado['empresaId'] . '/usuarios'); ?>" method="get">
    ...
    <?php if ($pagina < $paginasTotal) { ?>
      <a href="<?php echo baseUrl('/dashboard/' . $this->usuarioLogado['empresaId'] . '/usuarios?pagina=' . $pagina + 1); ?>" class="border-2 border-slate-100 hover:border-slate-500 text-gray-500 p-3 rounded-lg">
