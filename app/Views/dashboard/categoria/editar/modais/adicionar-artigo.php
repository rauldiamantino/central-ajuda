<dialog class="border border-slate-300 w-full md:w-[500px] rounded-md shadow ease-in-out menu-editar-categoria-novo-artigo">
  <form method="POST" action="<?php echo baseUrl('/' . $this->usuarioLogado['subdominio'] . '/d/artigo'); ?>" class="border border-slate-200 w-full flex flex-col gap-4 p-4 rounded-lg shadow bg-white">
    <input type="hidden" name="referer" value="<?php echo baseUrl('/' . $this->usuarioLogado['subdominio'] . '/dashboard/categoria/editar/' . $categoria[0]['Categoria']['id']) ?>">
