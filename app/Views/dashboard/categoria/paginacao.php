...
$filtrarNome = isset($filtroAtual['nome']) ? '&nome=' . urlencode($filtroAtual['nome']) : '';
?>
...
<div class="mt-2 py-2 h-max flex flex-col gap-2 md:gap-1 md:flex-row justify-start items-center">
  <div class="flex justify-center items-center gap-1">
    <?php if ($pagina > 1) { ?>
      <a href="<?php echo baseUrl('/' . $this->usuarioLogado['subdominio'] . '/dashboard/categorias'); ?>?pagina=<?php echo $pagina - 1 . $filtrarId . $filtrarStatus . $filtrarNome; ?>" class="border-2 border-slate-200 hover:border-slate-500 text-gray-500 p-3 rounded-lg">
    ...
    <?php } ?>
    <form action="<?php echo baseUrl('/' . $this->usuarioLogado['subdominio'] . '/dashboard/categorias'); ?>" method="get">
      ...
      <?php if ($filtrarNome) { ?>
        <input type="hidden" name="nome" value="<?php echo urlencode($filtroAtual['nome']) ?>">
      <?php } ?>
    </form>
