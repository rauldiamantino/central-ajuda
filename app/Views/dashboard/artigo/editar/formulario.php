<form method="POST" action="/<?php echo $this->usuarioLogado['subdominio'] ?>/d/artigo/<?php echo $artigo['Artigo.id'] ?>" class="border border-slate-200 w-full md:min-w-96 flex flex-col gap-4 p-4 rounded-lg shadow">
  ...
  <div class="w-full flex justify-start gap-2">
    <a href="/<?php echo $this->usuarioLogado['subdominio'] ?>/dashboard/artigos" class="w-full md:w-max border border-slate-400 flex gap-2 items-center justify-center py-2 px-3 hover:bg-slate-50 text-xs text-gray-700 rounded-lg">Cancelar</a>
