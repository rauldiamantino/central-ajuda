        ...
        <?php foreach ($conteudos as $chave => $linha) : ?>
          <div class="relative p-1 w-full h-full border-2 border-transparent group hover:border-black rounded div-pai-conteudo-editar">
            ...
            if ($linha['Conteudo']['tipo'] == 1) {
              $cliqueModalAbrir = 'onclick="window.location.href=\'' . baseUrl('/' . $this->usuarioLogado['subdominio'] . '/dashboard/conteudo/editar/' . $linha['Conteudo']['id'] . '?referer=' . $botaoVoltar) . '\';"';
            }
            ...
            ?>
            <div class="w-max h-full absolute top-0 -right-12 px-3 gap-5 hidden group-hover:flex flex-col lg:justify-center rounded">
              <button type="button" class="w-max h-max text-blue-800 flex items-center hover:text-blue-600 text-xs rounded-lg group" data-conteudo-id="<?php echo $linha['Conteudo']['id'] ?>" data-conteudo-url="<?php echo $linha['Conteudo']['url'] ?>" data-conteudo-tipo="<?php echo $linha['Conteudo']['tipo'] ?>" data-conteudo-titulo="<?php echo $linha['Conteudo']['titulo'] ?>" data-ordem-prox="<?php echo $ordem['prox'] ?? 0; ?>" data-conteudo-titulo-ocultar="<?php echo $linha['Conteudo']['titulo_ocultar']; ?>" <?php echo $cliqueModalAbrir; ?>>
