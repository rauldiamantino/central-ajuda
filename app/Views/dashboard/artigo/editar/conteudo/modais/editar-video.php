  ...
  <form method="POST" action="<?php echo baseUrl('/' . $this->usuarioLogado['subdominio'] . '/d/conteudo/' . $linha['Conteudo']['id']); ?>" class="flex flex-col items-end gap-2" enctype="multipart/form-data">
    ...
    <div class="sticky bottom-0 px-2 py-4 w-full h-max flex flex-col md:flex-row items-end justify-end gap-1 md:gap-4 bg-white rounded-lg">
      ...
      <div class="w-full flex flex-col sm:flex-row gap-1 justify-end sm:gap-2 bg-white">
        ...
        <button type="button" class="<?php echo CLASSES_DASH_BUTTON_VOLTAR; ?>" data-conteudo-id="<?php echo $linha['Conteudo']['id'] ?>" onclick="fecharVideoModal(event)">Cancelar</button>
