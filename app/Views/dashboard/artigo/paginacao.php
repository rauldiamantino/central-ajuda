...
<div class="sticky inset-x-0 bottom-0 border-t border-slate-200 py-2 w-full h-max flex flex-col gap-2 md:gap-1 md:flex-row justify-center items-center bg-white">
  <div class="flex justify-center items-center gap-1">
    <?php if ($pagina > 1) { ?>
      <a href="/dashboard/artigos?<?php echo $filtroAtual ? 'categoria_id=' . $filtroAtual . '&' : '' ?>pagina=<?php echo $pagina - 1 ?>" class="border border-slate-100 bg-slate-100 hover:bg-slate-200 text-gray-500 hover:text-black p-3 rounded-lg">
    ...
    <form action="/dashboard/artigos" method="get">
      <?php if ($filtroAtual) { ?>
        <input type="hidden" name="categoria_id" value="<?php echo $filtroAtual ?>">
      <?php } ?>
