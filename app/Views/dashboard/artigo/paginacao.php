...
$filtrarTitulo = isset($filtroAtual['titulo']) ? '&titulo=' . urlencode($filtroAtual['titulo']) : '';
$filtrarCategoriaId = isset($filtroAtual['categoria_id']) && $filtroAtual['categoria_id'] >= 0 ? '&categoria_id=' . $filtroAtual['categoria_id'] : '';
$filtrarCategoriaNome = isset($filtroAtual['categoria_nome']) && $filtroAtual['categoria_nome'] >= 0 ? '&categoria_nome=' . urlencode($filtroAtual['categoria_nome']) : '';
?>
...
<div class="mt-2 py-2 h-max flex flex-col gap-2 md:gap-1 md:flex-row justify-start items-center">
  <div class="flex justify-center items-center gap-1">
    <?php if ($pagina > 1) { ?>
      <a href="<?php echo baseUrl('/' . $this->usuarioLogado['subdominio'] . '/dashboard/artigos'); ?>?pagina=<?php echo $pagina - 1 . $filtrarId . $filtrarStatus . $filtrarTitulo . $filtrarCategoriaId . $filtrarCategoriaNome; ?>" class="border-2 border-slate-200 hover:border-slate-500 text-gray-500 p-3 rounded-lg">
    ...
    <form action="<?php echo baseUrl('/' . $this->usuarioLogado['subdominio'] . '/dashboard/artigos'); ?>" method="get">
      ...
      <?php if ($filtrarTitulo) { ?>
        <input type="hidden" name="titulo" value="<?php echo urlencode($filtroAtual['titulo']) ?>">
      <?php } ?>

      <?php if ($filtrarCategoriaId) { ?>
        <input type="hidden" name="categoria_id" value="<?php echo $filtroAtual['categoria_id'] ?>">
      <?php } ?>

      <?php if ($filtrarCategoriaNome) { ?>
        <input type="hidden" name="categoria_nome" value="<?php echo urlencode($filtroAtual['categoria_nome']) ?>">
      <?php } ?>
    </form>
