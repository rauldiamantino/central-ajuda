...
<div class="w-full flex flex-col py-14">
  ...
  <div class="w-full flex justify-start items-center gap-2 pt-6 publico-artigo-blocos publico-artigo-topo">

    <?php if ($categoriaNome and $categoriaIcone and $this->iconeExiste($categoriaIcone)) { ?>
      <div class="flex-shrink-0 w-8 pers-publico-icones template-cor-<?php echo Helper::ajuste('publico_cor_primaria'); ?>">
        <?php echo $this->renderIcone($categoriaIcone); ?>
      </div>
    <?php } elseif ($categoriaNome) { ?>
      <span class="flex-shrink-0 pers-publico-icones template-cor-<?php echo Helper::ajuste('publico_cor_primaria'); ?>">
    ...
    <?php } ?>
    <h1 class="text-base leading-none publico-artigo-titulo"><?php echo $categoriaNome ?></h1>
  </div>

  <div class="mt-6 w-full text-lg font-extralight">
        ...
        <?php foreach ($artigos as $chave => $linha): ?>
          <li class="flex gap-3 items-center rounded-lg bg-white shadow">
            <a href="<?php echo'/artigo/' . $linha['Artigo']['codigo'] . '/' . $this->gerarSlug($linha['Artigo']['titulo']); ?>" class="flex gap-2 justify-between items-center w-full h-full px-4 py-2 hover:bg-gray-100 rounded-md transition-colors duration-200 text-gray-800">
              ...
              <span class="pers-publico-seta-artigo template-cor-<?php echo Helper::ajuste('publico_cor_primaria'); ?>">
...
<?php if ($this->usuarioLogado['id'] and $this->usuarioLogado['subdominio'] and $this->usuarioLogado['empresaId'] and $this->usuarioLogado['empresaId'] == $this->empresaPadraoId) { ?>
  <div class="fixed bottom-32 right-4 sm:right-6 md:right-10 z-20 rounded-full hover:scale-110 duration-100 pers-publico-botao-editar template-cor-<?php echo Helper::ajuste('publico_cor_primaria'); ?>">
