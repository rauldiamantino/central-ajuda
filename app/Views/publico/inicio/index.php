  ...
  <div class="w-full flex flex-col gap-2">
    ...
    <div class="w-full h-max grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 justify-start gap-4">
      <?php foreach ($categorias as $chave => $linha): ?>
        <a href="<?php echo baseUrl('/' . $subdominio . '/categoria/' . $linha['Categoria']['id'] . '/' . $this->gerarSlug($linha['Categoria']['nome'])); ?>" class="border border-slate-200 shadow p-5 h-full min-h-[200px] bg-white flex flex-col gap-4 justify-center items-center text-center rounded-lg hover:scale-105 duration-100">
