...
<aside class="fixed z-40 md:z-10 inset-y-0 left-0 transform -translate-x-full md:translate-x-0 transform-translate duration-300 md:static w-full md:max-w-80 min-h-full bg-white text-black overflow-y-auto publico-menu-lateral md:rounded-l-md">
    ...
    <ul class="h-full py-10 border-r border-slate-200 px-4 flex flex-col gap-2">
      ...
      <?php if (isset($categorias[0]) and is_array($categorias[0])) { ?>
        ...
        <?php foreach ($categorias as $chave => $linha) : ?>
          <li class="w-full flex hover:underline <?php echo $categoriaAtual == $linha['Categoria.id'] ? ' font-bold' : '';?> hover:bg-slate-100 hover:text-black rounded-md">
      ...
      <?php if (isset($demaisArtigos) and $demaisArtigos) { ?>
        ...
        <?php foreach ($demaisArtigos as $chave => $linha) : ?>
          <li class="w-full flex hover:underline <?php echo $artigoAtual == $linha['Artigo.id'] ? ' font-bold' : '';?> hover:bg-slate-100 hover:text-black rounded-md">
      ...
      <h3 class="w-full text-start mt-10 border-t border-slate-100 mb-2 px-6 py-8 text-lg text-gray-600 font-light text-xs md:hidden">MENU</h3>
      <div class="w-full px-6 md:hidden">
