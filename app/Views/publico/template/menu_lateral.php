    ...
    <ul class="h-full border-r border-slate-300 px-4 py-2 flex flex-col">
      ...
      <?php if (isset($demaisArtigos) and $demaisArtigos) { ?>
        ...
        <?php foreach ($demaisArtigos as $chave => $linha) : ?>
          <li class="w-full flex hover:underline hover:bg-slate-100"><a href="/publico/artigo/<?php echo $linha['Artigo.id'] ?>" class="w-full px-6 py-4"><?php echo $linha['Artigo.titulo'] ?></a></li>
        <?php endforeach; ?>
      <?php } ?>
    </ul>
  </nav>
