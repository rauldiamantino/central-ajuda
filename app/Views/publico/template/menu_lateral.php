<aside class="fixed z-40 md:z-10 inset-y-0 left-0 transform -translate-x-full md:translate-x-0 transform-translate duration-100 md:static w-full md:max-w-80 min-h-full bg-gray-800 text-gray-300 overflow-y-auto publico-menu-lateral md:rounded-l-md">
    ...
    <ul class="h-full py-10 border-r border-slate-200 px-4 flex flex-col">
      <?php if (isset($categorias[0]) and is_array($categorias[0])) { ?>
        ...
        <?php foreach ($categorias as $chave => $linha) : ?>
          <li class="w-full flex hover:underline hover:bg-slate-100 hover:text-gray-800 rounded-md">
