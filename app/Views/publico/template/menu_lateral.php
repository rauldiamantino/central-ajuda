...
<aside class="fixed z-40 md:z-10 inset-y-0 left-0 transform -translate-x-full md:translate-x-0 transform-translate duration-300 md:static w-full md:max-w-80 min-h-screen bg-white text-gray-800 md:bg-white md:text-black overflow-y-auto md:rounded-l-md overflow-estilo publico-menu-lateral">
    ...
    <ul class="h-full py-10 border-r border-slate-200 px-4 flex flex-col">
      ...
      <?php if (isset($categorias[0]) and is_array($categorias[0])) { ?>
        ...
        <?php foreach ($categorias as $chave => $linha) : ?>
          <li class="w-full flex <?php echo $categoriaAtual == $linha['Categoria']['id'] ? ' font-bold' : '';?> hover:underline rounded-md">
            <a href="<?php echo baseUrl('/' . $subdominio . '/categoria/' . $linha['Categoria']['id']); ?>" class="w-full flex gap-2 items-center px-6 py-3">
      ...
      <?php if (isset($demaisArtigos) and $demaisArtigos) { ?>
        ...
        <?php foreach ($demaisArtigos as $chave => $linha) : ?>
          <li class="w-full flex <?php echo $artigoAtual == $linha['Artigo']['id'] ? ' font-bold' : '';?> hover:underline rounded-md">
            <a href="<?php echo baseUrl('/' . $subdominio . '/artigo/' . $linha['Artigo']['id']); ?>" class="w-full flex items-center gap-2 px-6 py-3"><?php echo $linha['Artigo']['titulo'] ?>
      ...
      <div class="w-full px-6 pb-10 md:hidden">
        <ul class="flex justify-start h-full gap-6">
          <li><a href="<?php echo baseUrl('/login'); ?>" target="_blank" class="hover:underline">Dashboard</a></li>
