...
<aside class="fixed <?php echo isset($inicio) ? ' md:hidden ' : '' ?> z-40 md:z-10 inset-y-0 left-0 transform -translate-x-full md:translate-x-0 transform-translate duration-300 md:static w-full md:max-w-80 min-h-screen bg-gray-800 text-gray-400 md:bg-white md:text-black overflow-y-auto md:rounded-l-md overflow-estilo publico-menu-lateral">
  <nav class="relative h-full text-sm">
    ...
    <ul class="h-full py-10 border-r border-slate-200 px-0 flex flex-col">

      <?php if (isset($categorias[0]) and is_array($categorias[0])) { ?>
        <h3 class="w-full text-start mb-2 px-4 py-4 text-lg text-gray-600 font-light text-xs">CATEGORIAS</h3>
      ...
      <?php if (isset($demaisArtigos) and $demaisArtigos) { ?>
        <h3 class="w-full text-start mb-2 px-4 py-4 text-lg text-gray-600 font-light text-xs">ARTIGOS RELACIONADOS</h3>
      ...
      <h3 class="w-full text-start mt-10 border-t border-gray-900 mb-2 px-4 py-8 text-lg text-gray-600 font-light text-xs md:hidden">MENU</h3>
